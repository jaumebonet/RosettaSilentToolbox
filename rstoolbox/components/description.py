    ...
    def check_graft_columns( self, headers ):
        if self.graft_ranges is None or self.graft_ranges == 1:
            ...
            for i in range(len(headers)):
                if re.search(graftsc, headers[i]) or headers[i] == 'graft_scaffold_size_change':
                    newnames = []
                    for j in range(self.graft_ranges):
                        newnames.append('{0}{1:02d}'.format(headers[i], j + 1))
                    if isinstance(self.scores, list) and headers[i] in self.scores:
                        self.scores.extend(newnames)
                    if isinstance(self.scores_ignore, list) and headers[i] in self.scores_ignore:
